...
('COMP_001', 'Veteran SSN Not Null', 'Veteran SSN must not be null', 'COMPLETENESS', 'CRITICAL', 'BUSINESS_RULE', 'staging', 'stg_veterans', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans') WHERE veteran_ssn IS NULL', '0', 0, 'DAILY', TRUE),
('COMP_002', 'Exam Request ID Not Null', 'Exam request ID must be present', 'COMPLETENESS', 'CRITICAL', 'BUSINESS_RULE', 'staging', 'stg_fact_exam_requests', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_fact_exam_requests') WHERE exam_request_id IS NULL', '0', 0, 'DAILY', TRUE),
('COMP_003', 'Evaluator NPI Not Null', 'Evaluator NPI must not be null', 'COMPLETENESS', 'HIGH', 'BUSINESS_RULE', 'staging', 'stg_evaluators', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_evaluators') WHERE evaluator_npi IS NULL', '0', 0, 'DAILY', TRUE),
('COMP_004', 'Critical Columns Completeness', 'All critical columns must be 95%+ complete', 'COMPLETENESS', 'HIGH', 'BUSINESS_RULE', 'staging', 'stg_veterans', 'SELECT (COUNT(*) - COUNT(veteran_ssn) - COUNT(first_name) - COUNT(last_name)) * 100.0 / NULLIF(COUNT(*), 0) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans')', '<5', 5, 'DAILY', TRUE);
...
('ACC_001', 'Valid SSN Format', 'SSN must be 9 digits', 'ACCURACY', 'CRITICAL', 'PATTERN', 'staging', 'stg_veterans', 'veteran_ssn', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans') WHERE veteran_ssn NOT REGEXP ''^[0-9]{9}$''', '0', 0, 'DAILY', TRUE),
('ACC_002', 'Valid NPI Format', 'NPI must be 10 digits', 'ACCURACY', 'HIGH', 'PATTERN', 'staging', 'stg_evaluators', 'evaluator_npi', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_evaluators') WHERE evaluator_npi NOT REGEXP ''^[0-9]{10}$''', '0', 0, 'DAILY', TRUE),
('ACC_003', 'Disability Rating Range', 'Disability rating must be between 0 and 100', 'ACCURACY', 'HIGH', 'DATA_TYPE', 'staging', 'stg_veterans', 'disability_rating', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans') WHERE disability_rating NOT BETWEEN 0 AND 100', '0', 0, 'DAILY', TRUE),
('ACC_004', 'Valid Email Format', 'Email must be valid format', 'ACCURACY', 'MEDIUM', 'PATTERN', 'staging', 'stg_veterans', 'email', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans') WHERE email IS NOT NULL AND email NOT REGEXP ''^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Z|a-z]{2,}$''', '0', 1, 'DAILY', TRUE),
('ACC_005', 'Future Dates Invalid', 'Dates must not be in the future', 'ACCURACY', 'HIGH', 'BUSINESS_RULE', 'staging', 'stg_fact_exam_requests', 'request_date', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_fact_exam_requests') WHERE request_date > CURRENT_DATE()', '0', 0, 'DAILY', TRUE);

-- CONSISTENCY RULES --

INSERT INTO dq_rule_catalog (rule_code, rule_name, rule_description, quality_dimension, severity, rule_category, target_schema, target_table, rule_sql, expected_result, failure_threshold, check_frequency, is_active) VALUES
('CONS_001', 'Veteran ID Exists in Dimension', 'All veteran IDs in facts must exist in dimension', 'CONSISTENCY', 'CRITICAL', 'REFERENTIAL_INTEGRITY', 'warehouse', 'fact_exam_requests', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.warehouse.fact_exam_requests') f WHERE NOT EXISTS (SELECT 1 FROM IDENTIFIER(get_dw_database() || '.warehouse.dim_veteran') d WHERE f.veteran_dim_sk = d.veteran_sk)', '0', 0, 'DAILY', TRUE),
('CONS_002', 'Evaluator ID Consistency', 'Evaluator IDs must match between OMS and VEMS', 'CONSISTENCY', 'HIGH', 'BUSINESS_RULE', 'staging', 'stg_evaluators', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_evaluators') o JOIN IDENTIFIER(get_dw_database() || '.staging.stg_evaluators') v ON o.evaluator_npi = v.evaluator_npi WHERE o.source_system = ''OMS'' AND v.source_system = ''VEMS'' AND o.first_name != v.first_name', '0', 5, 'DAILY', TRUE),
('CONS_003', 'Date Key Referential Integrity', 'All date keys must exist in dim_date', 'CONSISTENCY', 'CRITICAL', 'REFERENTIAL_INTEGRITY', 'warehouse', 'fact_exam_requests', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.warehouse.fact_exam_requests') f WHERE NOT EXISTS (SELECT 1 FROM IDENTIFIER(get_dw_database() || '.warehouse.dim_date') d WHERE f.request_date_sk = d.date_sk)', '0', 0, 'DAILY', TRUE);

-- TIMELINESS RULES --

INSERT INTO dq_rule_catalog (rule_code, rule_name, rule_description, quality_dimension, severity, rule_category, target_schema, target_table, rule_sql, expected_result, failure_threshold, check_frequency, is_active) VALUES
('TIME_001', 'ODS Data Freshness', 'ODS data must be refreshed within 24 hours', 'TIMELINESS', 'HIGH', 'BUSINESS_RULE', 'ods', 'ods_veterans_source', 'SELECT CASE WHEN MAX(created_timestamp) >= CURRENT_TIMESTAMP() - INTERVAL ''24 HOURS'' THEN 0 ELSE 1 END FROM IDENTIFIER(get_ods_database() || '.VEMS_CORE.veterans')', '0', 0, 'HOURLY', TRUE),
('TIME_002', 'Staging Load Timeliness', 'Staging must be updated within 2 hours of ODS', 'TIMELINESS', 'HIGH', 'BUSINESS_RULE', 'staging', 'stg_veterans', 'SELECT CASE WHEN MAX(updated_timestamp) >= CURRENT_TIMESTAMP() - INTERVAL ''2 HOURS'' THEN 0 ELSE 1 END FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans')', '0', 0, 'HOURLY', TRUE),
('TIME_003', 'Fact Table Refresh SLA', 'Fact tables must refresh daily by 8 AM', 'TIMELINESS', 'CRITICAL', 'BUSINESS_RULE', 'warehouse', 'fact_exam_requests', 'SELECT CASE WHEN MAX(created_timestamp)::DATE = CURRENT_DATE() THEN 0 ELSE 1 END FROM IDENTIFIER(get_dw_database() || '.warehouse.fact_exam_requests') WHERE created_timestamp::TIME < ''08:00:00''', '0', 0, 'DAILY', TRUE);

-- VALIDITY RULES --

INSERT INTO dq_rule_catalog (rule_code, rule_name, rule_description, quality_dimension, severity, rule_category, target_schema, target_table, rule_sql, expected_result, failure_threshold, check_frequency, is_active) VALUES
('VAL_001', 'Valid Military Branch', 'Military branch must be in approved list', 'VALIDITY', 'MEDIUM', 'BUSINESS_RULE', 'staging', 'stg_veterans', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans') WHERE military_branch NOT IN (''Army'', ''Navy'', ''Air Force'', ''Marines'', ''Coast Guard'', ''Space Force'')', '0', 1, 'DAILY', TRUE),
('VAL_002', 'Valid State Code', 'State must be valid 2-letter code', 'VALIDITY', 'MEDIUM', 'PATTERN', 'staging', 'stg_veterans', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_veterans') WHERE state NOT REGEXP ''^[A-Z]{2}$''', '0', 1, 'DAILY', TRUE),
('VAL_003', 'Valid Exam Status', 'Exam status must be in approved workflow states', 'VALIDITY', 'HIGH', 'BUSINESS_RULE', 'staging', 'stg_fact_exam_requests', 'SELECT COUNT(*) FROM IDENTIFIER(get_dw_database() || '.staging.stg_fact_exam_requests') WHERE exam_status NOT IN (''Requested'', ''Assigned'', ''Scheduled'', ''In Progress'', ''Completed'', ''Cancelled'')', '0', 0, 'DAILY', TRUE);

-- UNIQUENESS RULES --

INSERT INTO dq_rule_catalog (rule_code, rule_name, rule_description, quality_dimension, severity, rule_category, target_schema, target_table, rule_sql, expected_result, failure_threshold, check_frequency, is_active) VALUES
('UNQ_001', 'Veteran SSN Uniqueness', 'Active veteran records must have unique SSN', 'UNIQUENESS', 'CRITICAL', 'BUSINESS_RULE', 'warehouse', 'dim_veteran', 'SELECT COUNT(*) - COUNT(DISTINCT veteran_ssn) FROM IDENTIFIER(get_dw_database() || '.warehouse.dim_veteran') WHERE is_current = TRUE', '0', 0, 'DAILY', TRUE),
('UNQ_002', 'Exam Request ID Uniqueness', 'Exam request IDs must be unique', 'UNIQUENESS', 'CRITICAL', 'BUSINESS_RULE', 'warehouse', 'fact_exam_requests', 'SELECT COUNT(*) - COUNT(DISTINCT exam_request_sk) FROM IDENTIFIER(get_dw_database() || '.warehouse.fact_exam_requests')', '0', 0, 'DAILY', TRUE),
('UNQ_003', 'Evaluator NPI Uniqueness', 'Active evaluator NPIs must be unique', 'UNIQUENESS', 'CRITICAL', 'BUSINESS_RULE', 'warehouse', 'dim_evaluator', 'SELECT COUNT(*) - COUNT(DISTINCT evaluator_npi) FROM IDENTIFIER(get_dw_database() || '.warehouse.dim_evaluator') WHERE is_current = TRUE', '0', 0, 'DAILY', TRUE);
