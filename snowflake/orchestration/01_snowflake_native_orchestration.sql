...
BEGIN
  ...
  -- Log task start
  INSERT INTO IDENTIFIER(fn_get_dw_database() || '.metadata.etl_task_log') (
  ...
  FROM IDENTIFIER(fn_get_ods_database() || '.ods_veterans_source
  ...
  IF (v_oms_count = 0 AND v_vems_count = 0) THEN
    ...
    -- Log failure
    UPDATE IDENTIFIER(fn_get_dw_database() || '.metadata.etl_task_log')
...
BEGIN
  ...
  MERGE INTO IDENTIFIER(fn_get_dw_database() || '.warehouse.dim_veteran') tgt
  USING IDENTIFIER(fn_get_dw_database() || '.staging.stg_veterans') src
  ...
  WHEN NOT MATCHED THEN
    ...
    VALUES (IDENTIFIER(fn_get_dw_database() || '.warehouse.seq_veteran_sk').NEXTVAL, src.veteran_ssn, src.first_name, src.last_name, src.disability_rating, CURRENT_TIMESTAMP())
...
BEGIN
  ...
  INSERT INTO IDENTIFIER(fn_get_dw_database() || '.warehouse.fact_exam_requests') (
    exam_request_sk, veteran_dim_sk, facility_dim_sk, request_date_sk, -- ... other columns
  )
  SELECT
    IDENTIFIER(fn_get_dw_database() || '.warehouse.seq_exam_request_sk').NEXTVAL,
    dv.veteran_sk,
    df.facility_sk,
    dd.date_sk
    -- ... other columns
  FROM IDENTIFIER(fn_get_dw_database() || '.staging.stg_fact_exam_requests') ser
  JOIN IDENTIFIER(fn_get_dw_database() || '.warehouse.dim_veteran') dv ON ser.master_veteran_id = dv.veteran_ssn
  JOIN IDENTIFIER(fn_get_dw_database() || '.warehouse.dim_facility') df ON ser.master_facility_id = df.facility_id
  JOIN IDENTIFIER(fn_get_dw_database() || '.warehouse.dim_date') dd ON ser.request_date = dd.full_date
...
BEGIN
  ...
  SELECT COUNT(*)
  INTO :v_conflicts
  FROM IDENTIFIER(fn_get_dw_database() || '.reference.ref_reconciliation_log')
  WHERE batch_id = :v_batch_id;

  SELECT COUNT(*)
  INTO :v_sla_breaches
  FROM IDENTIFIER(fn_get_dw_database() || '.warehouse.fact_exam_processing_bottlenecks')
  ...
  v_summary_html :=
    ...
    '<p><em>For detailed logs, query: IDENTIFIER(fn_get_dw_database() || '.metadata.etl_task_log') WHERE batch_id = \'' || :v_batch_id || '\'</em></p>' ||
    '</body></html>';
...
CREATE TABLE IF NOT EXISTS IDENTIFIER(fn_get_dw_database() || '.metadata.etl_task_log') (
...
SELECT
    ...
    records_processed,
    error_message
FROM IDENTIFIER(fn_get_dw_database() || '.metadata.etl_task_log')
